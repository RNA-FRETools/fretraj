...
@pytest.mark.parametrize(
    "label_dict, missing_item, error",
    [(MINIMAL_LABELS, "Position", ValueError), (MINIMAL_LABELS["Position"]["Cy3"], "linker_length", KeyError)],
)
...
def test_parseCmd(mocker):
    mocker.patch(
        ...
        return_value=argparse.Namespace(input="test.pdb", parameters="param.json", site="Cy3", output=None),
    )
    out = console.parseCmd()
    assert out == ("test.pdb", "param.json", "Cy3", None)
